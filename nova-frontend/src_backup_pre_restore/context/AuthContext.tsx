...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const loadProfiles = async (userId: string, userMeta?: any, retryCount = 0) => {
        if (retryCount > 1) {
            console.error('Failed to load or create profile after retry');
            setLoading(false);
            return;
        }
        ...
        if (error) {
            ...
            // If checking fails, don't retry blindly
            return;
        }
        ...
        if (!data || data.length === 0) {
            ...
            // Retry loading with incremented counter
            return loadProfiles(userId, userMeta, retryCount + 1);
        }
