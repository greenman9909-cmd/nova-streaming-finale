...
export default function Hero() {
    ...
    const getHeroLink = (item: FeaturedContent) => {
        if (item.type === 'movie') return `/watch/movie/${item.id}`;
        if (item.type === 'tv') return `/watch/tv/${item.id}`;
        return `/anime/watch/${item.id}`;
    };

    const getHeroInfoLink = (item: FeaturedContent) => {
        if (item.type === 'anime') return `/anime/${item.id}`;
        return getHeroLink(item);
    };

    const isSaved = isInWatchlist(current.id);
    const watchType = current.type === 'movie' ? 'movie' : 'tv';
    const watchCategory = current.type === 'anime' ? 'Anime' : (current.type === 'movie' ? 'Movie' : 'Series');
                    ...
                    <div key={currentIndex} className="max-w-xl animate-fade-in-up">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-6 py-2.5 backdrop-blur text-white font-semibold text-sm rounded-lg border transition-all ${isSaved ? 'bg-white text-black border-white hover:bg-gray-200' : 'bg-white/10 border-white/20 hover:bg-white/20'}`}
                            ...
                            <button
                                onClick={() => navigate(getHeroInfoLink(current))}
                                className="w-10 h-10 rounded-full bg-white/10 backdrop-blur flex items-center justify-center text-white border border-white/20 hover:bg-white/20 transition-all"
                                title="Más info"
                            >
