            ...
            try {
                ...
                if (selectedCategory === 'all') {
                    ...
                    const animeItems: AvatarItem[] = (topAnimeChars || []).map((char: any) => ({
                        ...
                        subtitle: 'Anime'
                    })).filter(i => i.image && !i.image.includes('placeholder') && !i.image.includes('questionmark'));

                    const movieDrillItems: AvatarItem[] = (popularMovies || []).slice(0, 10).map((m: any) => ({
                        id: m.id,
                        name: m.title,
                        image: getImageUrl(m.poster_path, 'w500'),
                        type: 'media_tmdb' as const,
                        originalItem: m,
                        subtitle: 'Ver personajes'
                    })).filter(i => i.image && !i.image.includes('placeholder'));

                    const seriesDrillItems: AvatarItem[] = (popularSeries || []).slice(0, 10).map((s: any) => ({
                        id: s.id,
                        name: s.name,
                    ...
                    // Interleave: anime chars first, then movie/series drill-downs
                    mappedItems = [...animeItems, ...movieDrillItems, ...seriesDrillItems];
                }
                    ...
                    mappedItems = rawResults.map((char: any) => ({
                        ...
                        type: 'character_jikan' as const,
                        subtitle: 'Anime'
                    })).filter((i: AvatarItem) => i.image && !i.image.includes('placeholder') && !i.image.includes('questionmark'));
                }
