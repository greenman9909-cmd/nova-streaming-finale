...
export default function AnimeDetail() {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (infoData) {
                    ...
                    let resolvedSeason = seasons[0]?.season_number || 1;
                    let resolvedEpisodes: TMDBEpisode[] = [];

                    for (const season of seasons) {
                        const seasonEpisodes = await getSeasonEpisodes(contentId, season.season_number);
    ...
    const handleEpisodeClick = (episode: TMDBEpisode) => {
        navigate(`/anime/watch/${contentId}?season=${selectedSeason}&episode=${episode.episode_number}`);
    };
    ...
    const handleWatchlistToggle = async () => {
        ...
        await addToWatchlist({
            ...
            title: anime.name,
            category: 'Anime',
            image: getBackdropUrl(anime.backdrop_path) || getImageUrl(anime.poster_path),
            type: 'tv',
        ...
        <div className="min-h-screen bg-[#141414]">
                    ...
                    <div className="animate-fade-in-up">
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex items-center gap-4 text-white font-medium mb-6">
                                ...
                                <span>{anime.number_of_episodes} episodios</span>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <button
                                ...
                                className={`px-8 py-3 font-bold text-xl rounded flex items-center gap-3 transition-all ${isSaved
                                    ? 'bg-white text-black hover:bg-gray-200'
                                    : 'bg-[#6d6d6eb3] text-white hover:bg-[#6d6d6e80]'
                                    }`}
                                aria-pressed={isSaved}
                                title={isSaved ? 'Quitar de mi lista' : 'Añadir a mi lista'}
                            >
                                <i className={isSaved ? 'ri-check-line text-2xl' : 'ri-add-line text-2xl'}></i>
            ...
            <div className="relative z-30 -mt-10 px-6 md:px-12 pb-24 max-w-[1400px] mx-auto">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 border-t border-white/10 pt-10 gap-6">
                    <div>
                        <h3 className="text-3xl font-display font-bold text-white mb-2">Episodios</h3>
                        <p className="text-gray-400 text-sm">Temporada {selectedSeason} - {episodes.length} episodios</p>
                    </div>
                    ...
                    {episodes.map((episode) => (
                        ...
                        >
                            ...
                            <div className="relative aspect-video w-full rounded-xl overflow-hidden bg-[#232323] shadow-lg group-hover:shadow-violet-500/10 transition-all">
                                <img
                                    src={episode.still_path ? getImageUrl(episode.still_path) : getImageUrl(anime.backdrop_path || anime.poster_path)}
                                    ...
                                    onError={(e) => { (e.target as HTMLImageElement).src = getImageUrl(anime.poster_path); }}
                                />
